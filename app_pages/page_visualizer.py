...
def page_visualizer_body():
    ...
    st.info(
        ...
        f"differentiates a pizza from food that is not pizza.")

    version = 'v1'
    if st.checkbox("Difference between average and variability image"):

        avg_not_pizza = plt.imread(
            f"outputs/{version}/avg_var_not_pizza.png")
        ...
        st.image(avg_not_pizza,
                 caption='Not Pizza - Average and Variability')
        st.image(avg_pizza, caption='Pizza - Average and Variability')
        st.write("---")

    if st.checkbox("Differences between average pizza and food that is not pizza"):
