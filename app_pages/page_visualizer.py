...
def page_visualizer_body():
    ...
    st.info(
        "The client is interested in visually differentiating pizza from food that is not pizza."
    )
    ...
    if st.checkbox("Visualizing Pizza vs. Not-Pizza"):
        ...
        st.warning(
            "While exploring the visual differences, we couldn't find clear patterns to differentiate pizza from non-pizza. "
            "However, there's a subtle difference in color pigment between the average images of both labels."
        )
    ...
    if st.checkbox("Differences between Pizza and Not-Pizza Averages"):
        ...
        st.warning(
            "Our visual analysis didn't reveal distinct patterns that we could intuitively differentiate pizza from non-pizza images."
        )

        st.image(diff_between_avgs, caption='Difference between average images')

    if st.checkbox("Create Image Montage"):
        st.info("Click the 'Create Montage' button to generate an image montage.")
...
def image_montage(dir_path, label_to_display, nrows, ncols, figsize=(15, 10)):
    ...
    if label_to_display in labels:
        ...
        if nrows * ncols < len(images_list):
            ...
            st.warning(
                f"Decrease nrows or ncols to create your montage. There are {len(images_list)} images in your subset, "
